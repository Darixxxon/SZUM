...
for folder in os.listdir(path):
    ...
    if os.path.isdir(folder_path):
        fuit = folder.split('_')[0]
        if fuit not in count_fruit:
            count_fruit[fuit] = 0
        count_fruit[fuit] += len(os.listdir(folder_path))
        count[folder] = len(os.listdir(folder_path))
        if 'healthy' in folder_path:
            if fuit not in healthy_count:
                healthy_count[fuit] = 0
            healthy_count[fuit] += len(os.listdir(folder_path))
        else:
            if f"{fuit}_not_healthy" not in healthy_count:
                healthy_count[f"{fuit}_not_healthy"] = 0
            healthy_count[f"{fuit}_not_healthy"] += len(os.listdir(folder_path))
